...
path = r"C:\Users\ASUS\Desktop\git.repo\lab7\j-muz" 
MUZ = sorted(os.listdir(path)) 
 
pygame.init() 
    #screen 
pygame.display.set_caption("death player") 
...
pygame.mixer.music.load(os.path.join('j-muz', MUZ[0])) 
...
while not done: 
    ...
    if listnum != current: 
        pygame.mixer.music.load(os.path.join('j-muz', MUZ[listnum])) 
        pygame.mixer.music.play(0) 
